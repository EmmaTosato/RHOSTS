...
The `.npy` files contain nodal strength vectors (116 values, one per ROI). Brain map images must be generated on a local machine with display capabilities.
...
|-----------|---------|-------------|
| `current_nodestrength` | - | ndarray (116,) with nodal strength values |
